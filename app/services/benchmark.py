...
def _write_markdown(
    ...
    *,
    ...
    lines = [
        ...
        "Older-prompt model caches are historical-response replays under the displayed current "
        "profile/policy, not fresh Claude judgments of that profile. Legacy caches do not "
        "record their original profile version.",
        "",
...
def _evaluation_version_label(provenance: dict[str, str]) -> str:
    ...
    profile = provenance.get("candidate_profile_version") or "unknown"
    return f"{evaluator}; prompt={prompt}; model={model}; profile={profile}"
